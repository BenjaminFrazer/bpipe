...
int BpAggregatorPy_init(PyObject* self, PyObject* args, PyObject* kwds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "|iin", kwlist, &n_inputs,
                                     &dtype, &max_capacity_bytes)) {
        return -1;
    }

    if (n_inputs <= 0 || n_inputs > MAX_SOURCES) {
        PyErr_Format(PyExc_ValueError, "n_inputs must be between 1 and %d",
                     MAX_SOURCES);
        return -1;
    }
    ...
    if (max_capacity_bytes <= 0) {
        PyErr_SetString(PyExc_ValueError,
                        "max_capacity_bytes must be positive");
        return -1;
    }
    ...
    for (int i = 0; i < n_inputs; i++) {
        ec = AggregatorBuffer_Init(&agg->buffers[i], element_size, max_elements,
                                   (SampleDtype_t) dtype);
        if (ec != Bp_EC_OK) {
            ...
            free(agg->buffers);
            BpFilter_Deinit(&agg->base);
            PyErr_Format(PyExc_RuntimeError, "Failed to initialize buffer %d",
                         i);
            return -1;
        }
    }
