...
static const char* dtype_to_string(SampleDtype_t dtype) {
    switch (dtype) {
        ...
        case DTYPE_U32:   return "U32";
        case DTYPE_NDEF:  return "UNDEFINED";
...
static const char* overflow_to_string(OverflowBehaviour_t behaviour) {
    switch (behaviour) {
        case OVERFLOW_BLOCK: return "BLOCK";
        case OVERFLOW_DROP:  return "DROP";
        default:             return "UNKNOWN";
    }
}
...
static void print_batch(size_t idx, Batch_t* batch, bool is_head, bool is_tail, bool has_data) {
    char ts_buf[32];
    ...
    if (!has_data) {
        printf("│  [%3zu] %-30s%s                                   │",  /* 76->80: added 4 spaces */
               idx, "", markers);
        return;
    }
...
void bb_print(Batch_buff_t* buff) {
    ...
    size_t tail = atomic_load_explicit(&buff->consumer.tail, memory_order_relaxed);
    ...
    printf("║ Type: %-8s │ Batches: %4zu │ Batch Size: %4zu │ Overflow: %-10s     ║\n",  /* 75->80: added 5 spaces */
    ...
    printf("║ Head: %4zu      │ Tail: %4zu    │ Used: %4zu/%4zu │ Status: %-12s     ║\n",  /* 75->80: added 5 spaces */
    ...
    printf("║ Total: %-8llu │ Dropped: %-8llu │ Drop Rate: %5.1f%%                      ║\n",  /* 79->80: added 1 more space */
    ...
    /* Legend */
    printf("║ H=Head(Write) T=Tail(Read) │ [idx] ID:batch_id timestamp                     ║\n");  /* 79->80: added 1 space */
    printf("╠══════════════════════════════════════════════════════════════════════════════╣\n");
