...
Bp_EC BpFilter_Init_Core(Bp_Filter_t* filter, SampleDtype_t dtype, size_t n_inputs, size_t batch_size_expo, size_t n_batch_expo);
                                       ...
                                       unsigned long timeout_us)
{
    ...
    while (Bp_empty(buf) && !buf->stopped) {
        if (timeout_us == 0) {
            ...
            int ret = pthread_cond_timedwait(&buf->not_empty, &buf->mutex,
                                             &abs_timeout);
            if (ret == ETIMEDOUT) {
                ec = Bp_EC_TIMEOUT;
                break;
                                      ...
                                      unsigned long timeout_us)
{
    ...
    while ((buf->head - buf->tail) >= Bp_ring_capacity(buf) && !buf->stopped) {
        if (timeout_us == 0) {
            ...
            int ret = pthread_cond_timedwait(&buf->not_full, &buf->mutex,
                                             &abs_timeout);
            if (ret == ETIMEDOUT) {
                ec = Bp_EC_TIMEOUT;
                break;
