...
Bp_EC map_init(Map_filt_t* f, Map_config_t config)
{
  ...
  prop_append_behavior(&f->base, PROP_DATA_TYPE, BEHAVIOR_OP_SET, &dtype, OUTPUT_ALL);
  
  // Preserve sample period from input (or leave unknown if input is unknown)
  prop_append_behavior(&f->base, PROP_SAMPLE_PERIOD_NS, BEHAVIOR_OP_PRESERVE, NULL, OUTPUT_ALL);
  ...
  prop_append_behavior(&f->base, PROP_MIN_BATCH_CAPACITY, BEHAVIOR_OP_SET, &min_batch, OUTPUT_ALL);
  prop_append_behavior(&f->base, PROP_MAX_BATCH_CAPACITY, BEHAVIOR_OP_SET, &max_batch, OUTPUT_ALL);
