...
static void* passthrough_worker(void* arg)
{
  ...
  BP_WORKER_ASSERT(&pt->base, pt->base.n_input_buffers == 1,
  ...
  while (atomic_load(&pt->base.running)) {
    // Get input batch
    Batch_t* input =
        bb_get_tail(pt->base.input_buffers[0], pt->base.timeout_us, &err);
    if (!input) {
      if (err == Bp_EC_TIMEOUT) continue;
      if (err == Bp_EC_STOPPED) break;
      if (err == Bp_EC_FILTER_STOPPING) break;
      break;  // Real error
    }
  ...
  // Error handling
  if (err != Bp_EC_OK && err != Bp_EC_STOPPED && err != Bp_EC_FILTER_STOPPING) {
...
static Bp_EC passthrough_describe(Filter_t* self, char* buffer, size_t size)
{
  snprintf(buffer, size,
           ...
           self->name, self->metrics.n_batches,
...
Bp_EC passthrough_init(Passthrough_t* pt, Passthrough_config_t* config)
{
  ...
  // Validate config
  if (config->buff_config.dtype == DTYPE_NDEF ||
      config->buff_config.dtype >= DTYPE_MAX) {
