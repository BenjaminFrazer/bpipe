...
static Bp_EC sample_aligner_start(Filter_t* self) {
    ...
    // Start worker thread
    self->running = true;
    if (pthread_create(&self->worker_thread, NULL, self->worker, (void*)self) != 0) {
        self->running = false;
...
static void* sample_aligner_worker(void* arg) {
    ...
    while (f->running) {
        ...
        memcpy(output->data, 
               (char*)input->data + input->head * bb_getdatawidth(f->input_buffers[0].dtype),
...
static Bp_EC sample_aligner_describe(Filter_t* self, char* buffer, size_t size) {
    ...
    switch (sa->method) {
        case INTERP_NEAREST: method_str = "NEAREST"; break;
        case INTERP_LINEAR: method_str = "LINEAR"; break;
        case INTERP_CUBIC: method_str = "CUBIC"; break;
        case INTERP_SINC: method_str = "SINC"; break;
    }
    
    const char* align_str = "UNKNOWN";
    switch (sa->alignment) {
        case ALIGN_NEAREST: align_str = "NEAREST"; break;
        case ALIGN_BACKWARD: align_str = "BACKWARD"; break;
        case ALIGN_FORWARD: align_str = "FORWARD"; break;
    }
    ...
    snprintf(buffer, size, 
             ...
             self->name, method_str, align_str,
             sa->period_ns, sa->samples_interpolated,
             sa->max_phase_correction_ns);
