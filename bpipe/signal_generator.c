...
static void generate_sine(SignalGenerator_t* sg, float* samples, size_t n, uint64_t t_start_ns) {
...
static void generate_square(SignalGenerator_t* sg, float* samples, size_t n, uint64_t t_start_ns) {
    for (size_t i = 0; i < n; i++) {
        double t_ns = t_start_ns + i * sg->period_ns;
        double phase = sg->omega * t_ns + sg->initial_phase_rad;
...
static void generate_sawtooth(SignalGenerator_t* sg, float* samples, size_t n, uint64_t t_start_ns) {
    for (size_t i = 0; i < n; i++) {
        double t_ns = t_start_ns + i * sg->period_ns;
        double phase = sg->omega * t_ns + sg->initial_phase_rad;
        ...
        samples[i] = sg->amplitude * (2.0 * normalized / (2.0 * M_PI) - 1.0) + sg->offset;
    }
}

// Generate triangle waveform
static void generate_triangle(SignalGenerator_t* sg, float* samples, size_t n, uint64_t t_start_ns) {
    for (size_t i = 0; i < n; i++) {
        double t_ns = t_start_ns + i * sg->period_ns;
        double phase = sg->omega * t_ns + sg->initial_phase_rad;
        // Normalize phase to [0, 2π]
...
static void generate_waveform(SignalGenerator_t* sg, float* samples, size_t n, uint64_t t_start_ns) {
...
void* signal_generator_worker(void* arg) {
    ...
    if (!sg->allow_aliasing) {
        ...
        BP_WORKER_ASSERT(&sg->base, sg->frequency_hz <= nyquist_hz, Bp_EC_INVALID_CONFIG);
    }
    ...
    while (atomic_load(&sg->base.running)) {
        ...
        // Submit batch
        err = bb_submit(sg->base.sinks[0], 0);  // No timeout
        if (err != Bp_EC_OK) {
...
Bp_EC signal_generator_init(SignalGenerator_t* sg, SignalGenerator_config_t config) {
    ...
    if (config.waveform_type < WAVEFORM_SINE || config.waveform_type > WAVEFORM_TRIANGLE) {
        return Bp_EC_INVALID_CONFIG;
    }
