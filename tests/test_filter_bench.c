...
void test_connection_multi_sink(void) {
    ...
    ControllableConsumer_t* consumers = calloc(test_sinks + 1, sizeof(ControllableConsumer_t));
    ...
    for (size_t i = 0; i < test_sinks; i++) {
        ...
        TEST_ASSERT_EQUAL_MESSAGE(Bp_EC_OK, err, "Failed to connect within max_supported_sinks");
    ...
    if (test_sinks < max_sinks) {
        ...
        err = filt_sink_connect(g_fut, max_sinks, consumers[test_sinks].base.input_buffers[0]);
        TEST_ASSERT_EQUAL_MESSAGE(Bp_EC_INVALID_SINK_IDX, err, 
...
void test_dataflow_passthrough(void) {
    ...
    if (g_fut->n_input_buffers > 0) {
        ...
        for (int i = 0; i < g_fut->n_input_buffers; i++) {
            ...
            ControllableProducerConfig_t prod_config = {
                ...
                .pattern = PATTERN_SEQUENTIAL,
                .max_batches = 10,
                .start_sequence = 1000 + i * 1000  // Different sequences for each input
            };
    ...
    if (consumer) {
        ...
        if (producer) {
            ...
            size_t min_batches_produced = atomic_load(&producers[0]->batches_produced);
            ...
            TEST_ASSERT_INT_WITHIN_MESSAGE(2, min_batches_produced, batches_consumed,
    ...
    if (producers && !consumer) {
        ...
        for (int i = 0; i < g_fut->n_input_buffers; i++) {
            size_t batches_produced = atomic_load(&producers[i]->batches_produced);
...
void test_dataflow_backpressure(void) {
    ...
    if (producer && consumer) {
        ...
        TEST_ASSERT_LESS_THAN_MESSAGE(50, min_produced,
                                    "Producers should be throttled by backpressure");
