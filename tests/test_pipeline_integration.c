...
static void* test_sink_worker(void* arg)
{
    ...
    while (atomic_load(&self->running)) {
        Batch_t* batch = bb_get_head(self->input_buffers[0]);
        if (!batch) {
            continue;
        }
 ...
 * SignalGenerator -> Pipeline[Map1 -> Map2] -> TestSink
 */
void test_pipeline_linear_data_flow(void)
{
    ...
    Map_config_t scaler_config = {
        .name = "scaler",
        .buff_config = default_buffer_config(),
        .map_fcn = scale_by_2,
        .timeout_us = 1000000
    };
    Map_config_t offset_config = {
        .name = "offset",
        .buff_config = default_buffer_config(),
    ...
    CHECK_ERR(filt_sink_connect(&sig_gen.base, 0, pipeline.base.input_buffers[0]));
    ...
    TEST_ASSERT_GREATER_THAN(50, samples_received);  // At least 50 samples received

    /* Verify transformations: input * 2 + 10 */
    pthread_mutex_lock(&sink.mutex);
    for (int i = 0; i < 10 && i < samples_received; i++) {
 ...
 * SignalGenerator -> Pipeline[Tee -> (Scaler, Offset)] -> TestSinks
 */
void test_pipeline_dag_data_flow(void)
{
    ...
    Tee_filt_t splitter;
    BatchBuffer_config tee_out_configs[] = {
    ...
    CHECK_ERR(filt_sink_connect(&scaler.base, 0, sink_scaler.base.input_buffers[0]));
    CHECK_ERR(filt_sink_connect(&offset.base, 0, sink_offset.base.input_buffers[0]));

    /* Create DAG pipeline: tee -> (scaler, offset) */
    Filter_t* filters[] = {&splitter.base, &scaler.base, &offset.base};
    Connection_t connections[] = {
        {&splitter.base, 0, &scaler.base, 0},
    ...
    for (int i = 0; i < 20 && i < scaler_samples && i < offset_samples; i++) {
 ...
 * Verifies that pipelines can contain other pipelines
 */
void test_pipeline_nested(void)
{
    /* Create inner pipeline filters */
    Map_filt_t inner_scaler, inner_offset;
    Map_config_t inner_scaler_config = {
        .name = "inner_scaler",
        .buff_config = default_buffer_config(),
        .map_fcn = scale_by_2,
        .timeout_us = 1000000
    };
    Map_config_t inner_offset_config = {
        .name = "inner_offset",
        .buff_config = default_buffer_config(),
        .map_fcn = offset_by_10,
        .timeout_us = 1000000
    ...
    Pipeline_config_t inner_config = {
        .name = "inner_pipeline",
        .buff_config = default_buffer_config(),
        .timeout_us = 1000000,
