...
void* test_source_worker(void* arg)
{
  ...
  if (batch == NULL) {
    TEST_FAIL_MESSAGE(
        "bb_get_head returned NULL when sending completion - buffer might be "
        "full");
  }
...
void test_various_phase_offsets(void)
{
  ...
  for (size_t i = 0; i < sizeof(phase_offsets) / sizeof(phase_offsets[0]);
    ...
    TEST_ASSERT_EQUAL(phase_offsets[i],
                      fixture.aligner.max_phase_correction_ns);
...
void test_alignment_strategies(void)
{
  ...
  AlignmentStrategy_e strategies[] = {ALIGN_NEAREST, ALIGN_BACKWARD,
                                      ALIGN_FORWARD};
  ...
  for (size_t i = 0; i < sizeof(strategies) / sizeof(strategies[0]); i++) {
    ...
    TEST_ASSERT_EQUAL(0,
                      fixture.aligner.next_output_ns % fixture.test_period_ns);
